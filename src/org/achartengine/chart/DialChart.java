...
public class DialChart extends RoundChart {
  ...
  private static final int NEEDLE_RADIUS = 3;
  /** The series renderer. */
  ...
  public void draw(Canvas canvas, int x, int y, int width, int height, Paint paint) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      Type type = mRenderer.getVisualTypeForIndex(i);
      drawNeedle(canvas, angle, mCenterX, mCenterY, shortRadius, type, paint);
    }
    drawLegend(canvas, mRenderer, titles, left, right, y, width, height, legendSize, paint, false);
    drawTitle(canvas, x, y+y1sec, width, paint);
  }
